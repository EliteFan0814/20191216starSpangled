        ...
        <div v-for="item in tableData"
          ...
          <div class="itemC">
            ...
            <el-input style="float: left;"
              v-if="item.code === 'customer_service'"
              v-model="item.value"
    ...
    getData() {
      this.$http.get('/api/SystemSetting/List').then(res => {
        ...
        for (let i = 0; i < systemStr.length; i++) {
          if (
            !systemStr[i].code.indexOf('customer_service') ||
            !systemStr[i].code.indexOf('out_commission') ||
            !systemStr[i].code.indexOf('rank_lock') ||
            !systemStr[i].code.indexOf('speedup_lock')
          ) {
